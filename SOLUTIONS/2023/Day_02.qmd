...
Each draw contains number of balls that were selected in each color: `red`,
`green`, or `blue`.
...
function find_colors_numbers(draws::String, color::String)
    ...
    while idx < lastindex(draws)
        ...
        push!(out_num_balls, parse(Int32, m["num_balls"]));
        idx = m.offset + length(m.match);
    end
    return out_num_balls
end
```
